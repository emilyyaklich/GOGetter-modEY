...
best_hits_copy['Query'] = best_hits_copy['Query'].str.extract(r'([^\::]+)')


# rename columns for clarity
best_hits_S.columns = ['gene_name', 'TAIR_ID']

# clean the 'TAIR_ID' column by removing version numbers (e.g., AT2G07760.1 -> AT2G07760)
# this is needed to map to the GOSlim database
best_hits_S['TAIR_ID'] = best_hits_S['TAIR_ID'].str.replace(r'\.\d+$', '', regex=True)
...
GOSlim_DB_subset = GOSlim_DB[['TAIRAccession', 'GOID', 'GOTerm','EvidenceCode']].copy()  # make a copy to avoid SettingWithCopyWarning


# Check example sample cleaned data from both files
print("Sample Best Hits (TAIR IDs cleaned):")
print(best_hits_copy.head())

print("Sample GOSlim DB (TAIRAccession cleaned):")
print(GOSlim_DB_subset.head())

# merge based on cleaned-up 'TAIR_ID' (your genes) and 'TAIRAccession' (from GOSlim DB)
GOSlim_merge_gene = best_hits_copy.merge(GOSlim_DB_subset, 'inner', left_on='TAIR_ID', right_on='TAIRAccession')
